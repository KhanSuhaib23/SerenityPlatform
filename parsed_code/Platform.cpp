...
{
    map->size++;
    int32* newUserIndex = (int32*) malloc(sizeof(int32) * map->size);
    int32* newWindowIndex = (int32*) malloc(sizeof(int32) * map->size);
...
int32 AddWindow() //returns window index
{
    ...
    {
        environment.windows = (SerenityWindowPointer*) malloc(sizeof(SerenityWindowPointer));
        ...
        environment.windows[0] = (SerenityWindowPointer) malloc(sizeof(SerenityWindow));
        
        return 0;
    }
    else
    {
        SerenityWindowPointer* newWindows = (SerenityWindowPointer*) malloc(sizeof(SerenityWindowPointer) * (environment.numberOfWindows + 1));
        int32 windowIndex = environment.numberOfWindows++;
        int32 userIndex = environment.currentUserIndex++;
        Add(&environment.map, userIndex, windowIndex);
        ...
        newWindows[i] = (SerenityWindowPointer) malloc(sizeof(SerenityWindow));
...
{
    SerenityWindowPointer* newWindows = (SerenityWindowPointer*) (environment.numberOfWindows == 1 ? NULL : malloc(sizeof(SerenityWindowPointer) * (environment.numberOfWindows - 1)));
...
typedef HRESULT __SerenityCreateAudioDeviceSignature(IXAudio2** , UINT32 , XAUDIO2_PROCESSOR);
...
global __SerenityCreateAudioDeviceSignature* __SerenityCreateAudioDevice;
...
void SerenityInitialize()
{
    ...
    HMODULE xAudioDLL = LoadLibrary("xaudio2_7.dll");
    ...
    __SerenityCreateAudioDevice = (__SerenityCreateAudioDeviceSignature*) GetProcAddress(xAudioDLL, "XAudio2Create");
                                   ...
                                   RenderCallbackFunction* renderCallback)
{
    ...
    WNDCLASSEX windowClass = {}; // Creating a window class
    ...
    if (RegisterClassEx(&windowClass) != 0) // TODO(Suhaib): Maybe check unicode vs ASCII
    {
        ...
        if (windowHandle != NULL)
        {
            *userIndex = environment.currentUserIndex;
            ...
            window->waveInfo.nBlockAlign = 2 * 16 / 8;
            window->waveInfo.nAvgBytesPerSec = 48000 * 4;
            ...
            if( FAILED(hr = pXAudio2->CreateSourceVoice( &pSourceVoice, &window->waveInfo ) ) ) 
                return SERENITY_WINDOWCREATIONFAILED;
            
            if( FAILED(hr = pSourceVoice->SubmitSourceBuffer( &xAudioBuffer) ) )
                return SERENITY_WINDOWCREATIONFAILED;
...
{
    SerenityMousePosition mouse = {};
    mouse.windowIndex = environment.mouseHoverWindowIndex;
    mouse.xPos = environment.xPos;
    mouse.yPos = environment.yPos;
...
inline SerenityClock SerenityStartTimer()
{
    ...
    SerenityClock clock = {(float) time.QuadPart};
...
{
    SerenityCoordinate cood = { environment.controllers[index].lAnalogX, environment.controllers[index].lAnalogY };
    return cood;
}

inline SerenityCoordinate SerenityControllerRightAnalog(int index)
{
    SerenityCoordinate cood = { environment.controllers[index].rAnalogX, environment.controllers[index].rAnalogY };
    return cood;
}

inline void SerenityControllerVibrate(uint32 index, uint16 leftMotor, uint16 rightMotor)
{
    XINPUT_VIBRATION vibration = {leftMotor, rightMotor};
